...
def add_job(WORKFLOW, FILEPATH, config_dict, file_numbers_in_run = None):
	...
	if(FILENO != "-1" and file_numbers_in_run is None):
	...
	# inputs
	if file_numbers_in_run is None:
		add_command += " -input " + FILENAME + " " + INPUTDATA_TYPE + ":" + INDATA_DIR + "/" + FILENAME
	else:
		# add all files of a run as input files
		for file_number in file_numbers_in_run:
			file_name = PREFIX + "_" + RUNNO + "_" + file_number + "." + EXTENSION
			add_command += " -input " + file_name + " " + INPUTDATA_TYPE + ":" + INDATA_DIR + "/" + file_name
	# stdout, stderr
	if(FILENO != "-1" and file_numbers_in_run is None):
	...
	# command + arguments
	add_command += " " + config_dict["SCRIPTFILE"] + " " + config_dict["ENVFILE"] + " " + (FILENAME if file_numbers_in_run is None else PREFIX + "_" + RUNNO + "_'*'." + EXTENSION) + " " + JANA_CONFIG
	# command arguments continued
	add_command += " " + config_dict["OUTDIR_LARGE"] + " " + config_dict["OUTDIR_SMALL"] + " " + RUNNO + " " + (FILENO  if file_numbers_in_run is None else "000") + " " + CACHE_PIN_DAYS
