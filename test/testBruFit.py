...
if __name__ == "__main__":
  ...
  print(f">>> Reading weights '{weightBranchName}' from tree '{treeName}' in file '{templateDataFileName}'"
        f" and  writing them to key '{sigWeightObjectName}' in file '{sigWeightFileName}', and assigning label '{weightLabel}' while applying cut(s) '{sigCut}'")
  ...
  weights.SetSpecies(weightLabel)
  ...
  fitManager.SetUp().FactoryPDF(
    ...
    + f"WEIGHTS@{weightLabel},{sigWeightFileName},{sigWeightObjectName}"  # apply sWeights created above; !Note! no whitespace allowed in this string
  )
  ...
  print(f">>> Reading weights '{weightBranchName}' from tree '{treeName}' in file '{templateDataFileName}'"
        f" and  writing them to key '{bkgWeightObjectName}' in file '{bkgWeightFileName}', and assigning label '{weightLabel}' while applying cut(s) '{bkgCut}'")
  currentDir = ROOT.gDirectory
  inputFile = ROOT.TFile.Open(templateDataFileName, "READ")
  inputTree = inputFile.Get(treeName)
  currentDir.cd()
  ...
  fitManager.SetUp().FactoryPDF(
    ...
    + f"WEIGHTS@{weightLabel},{bkgWeightFileName},{bkgWeightObjectName}"  # apply sWeights created above; !Note! no whitespace allowed in this string
  )
  ...
  print(f">>> Reading weights '{weightBranchName}' from tree '{treeName}' in file '{dataFileName}'"
        f" and  writing them to key '{dataWeightObjectName}' in file '{dataWeightFileName}', and assigning label '{weightLabel}' while applying cut(s) '{cut}'")
  currentDir = ROOT.gDirectory
  ...
  fitManager.Data().LoadWeights(weightLabel, dataWeightFileName, dataWeightObjectName)
