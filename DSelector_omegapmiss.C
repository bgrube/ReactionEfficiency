...
#include "DSelector_omegapmiss.h"
...
void DSelector_omegapmiss::Init(TTree *locTree)
{
	...
	const string baseName = "omegapmiss";
	...
	dHist_SignalTruthThreePionMass                  = new TH1F("SignalTruthThreePionMass",                  ";Four-Pion Mass (GeV/c^{2})",   200,    0,    2);
	...
	dHist_SignalTruthThreePionMass_BeamEnergyRange1 = new TH1F("SignalTruthThreePionMass_BeamEnergyRange1", ";Four-Pion Mass (GeV/c^{2})",   200,    0,    2);
	...
	dHist_SignalTruthThreePionMass_BeamEnergyRange2 = new TH1F("SignalTruthThreePionMass_BeamEnergyRange2", ";Four-Pion Mass (GeV/c^{2})",   200,    0,    2);
...
void
DSelector_omegapmiss::setTreeKinematics(
...
void
DSelector_omegapmiss::setTreeDelta(
...
Bool_t DSelector_omegapmiss::Process(Long64_t locEntry)
{
	...
	*/
		...
		*/
		...
		for (UInt_t locTrackIndex = 0; locTrackIndex < Get_NumChargedHypos(); ++locTrackIndex) {
			...
			if ((dChargedHypoWrapper->Get_PID() != Proton)
			    or (dChargedHypoWrapper->Get_TrackID() == locPiPlusTrackID)  or (dChargedHypoWrapper->Get_TrackID() == locPiMinusTrackID)) {
				continue;
			}
			...
			locUsedThisCombo_UnusedTrack[PiPlus ].insert(locPiPlusTrackID);
			locUsedThisCombo_UnusedTrack[PiMinus].insert(locPiMinusTrackID);
			locUsedThisCombo_UnusedTrack[Gamma  ].insert(locPhoton1NeutralID);
			locUsedThisCombo_UnusedTrack[Gamma  ].insert(locPhoton2NeutralID);
	...
	if(dThrownBeam != NULL) {
		...
		if (locThrownTopology == "2#gamma#pi^{#plus}#pi^{#minus}p[#pi^{0},#omega]") {
			...
			assert(locPionsP4_Truth.size() == 3);
			TLorentzVector locThreePionP4_Truth;
			for (const auto& locPionP4 : locPionsP4_Truth) {
				locThreePionP4_Truth += locPionP4;
			}
			dHist_SignalTruthBeamEnergy->Fill   (locBeamEnergy_Truth);
			dHist_SignalTruthThreePionMass->Fill(locThreePionP4_Truth.M());
			...
			if ((5.5 < locBeamEnergy_Truth) and (locBeamEnergy_Truth < 11.0)) {
				...
				dHist_SignalTruthThreePionMass_BeamEnergyRange1->Fill(locThreePionP4_Truth.M());
			...
			if ((7.5 < locBeamEnergy_Truth) and (locBeamEnergy_Truth < 9.0)) {
				...
				dHist_SignalTruthThreePionMass_BeamEnergyRange2->Fill(locThreePionP4_Truth.M());
...
void DSelector_omegapmiss::Finalize(void)
{
